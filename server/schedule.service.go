...
// ScheduleCall assetps a list of rooms to call a certain time,
// saves them in Postgres and createds scheduler job if not exists yet
func (s *Server) ScheduleCall(w http.ResponseWriter, r *http.Request) {
	...
	for _, room := range rooms {
		...
		if room.Lastname == "" {
			s.error(w, r, fmt.Errorf("No last name provided for room number %d", room.RoomNumber), http.StatusBadRequest)
			return
		}
		if room.CallTime == "" {
